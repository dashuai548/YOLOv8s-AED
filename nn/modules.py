...
class DS_conv3(nn.Module):
    def __init__(self, dim, n_div, forward):
        '''
            n_div：代表隔几个取1个加入到需要卷积的序列中
            10266498 parameters, 10266482 gradients, 33.8 GFLOPs
        '''
        super().__init__()
        self.div=n_div
        self.dim_conv3 = math.ceil(dim / n_div)
    ...
    def forward_split_cat(self, x):
        group=torch.split(x,1,1)
        need_conv=group[0]
        no_conv=group[1]
        for i in range(2,self.div):
            no_conv=torch.cat((no_conv,group[i]),dim=1)
        for i in range(self.div,len(group),self.div):
            need_conv=torch.cat((need_conv,group[i]),dim=1)
            for j in range(i+1,self.div+i):
                if j<len(group):
                    no_conv=torch.cat((no_conv,group[j]),dim=1)
                else:
                    break
        need_conv=self.partial_conv3(need_conv)
        result=torch.cat((need_conv,no_conv),dim=1)
        return result
...
class C2f_Faster1(C2f):
    # C2f module with cross-convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = nn.Sequential(*(Faster_Block1(c_, c_) for _ in range(n)))


class C2f_Faster2(C2f):
    # C2f module with cross-convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = nn.Sequential(*(Faster_Block2(c_, c_) for _ in range(n)))
